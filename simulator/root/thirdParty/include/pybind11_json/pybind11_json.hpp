...
// 
// ASRC made a change to the original version as below:
// 1. Added direct conversion from numpy.ndarray to nl::json
// 2. Disabled automatic nl::json casting from/to Python primitives, in order to expose nl::json variables directly as "mutable" references.
//
...
//ASRC modification: added an include file to handle numpy.ndarray.
...
namespace pyjson
{
    ...
    inline nl::json to_json(const py::handle& obj)
    {
        ...
        //ASRC modification: added re-casting from "Pythonized" nl::json object.
        if (py::isinstance<nl::json>(obj))
        {
            return obj.cast<nl::json>();
        }
        ...
        if (py::isinstance<py::int_>(obj))
        {
            ...
            throw std::runtime_error("to_json received an integer out of range for both nl::json::number_integer_t and nl::json::unsigned_t type: " + py::repr(obj).cast<std::string>());
        }
        ...
        //ASRC modification: added direct conversion from numpy.ndarray object.
        if (py::isinstance<py::array>(obj))
        {
            return to_json(obj.attr("tolist")());
        }
...
namespace nlohmann
{
    ...
    //ASRC modification: added direct conversion from numpy.ndarray object.
    MAKE_NLJSON_SERIALIZER_ONLY(py::array);

    #undef MAKE_NLJSON_SERIALIZER
    #undef MAKE_NLJSON_SERIALIZER_ONLY
}
//ASRC modification: disabled automatic casting to/from python primitive objects.
/*
// pybind11 caster
namespace pybind11
{
