...
R4InitialFighterAgent01::R4InitialFighterAgent01(const nl::json& modelConfig_,const nl::json& instanceConfig_)
:SingleAssetAgent(modelConfig_,instanceConfig_){
    if(isDummy){return;}
    //失探後の航跡の保持に関するパラメータ
    tMaxMemory=getValueFromJsonKRD(modelConfig,"tMaxMemory",randomGen,10.0);
    //探知状況の分類に関するパラメータ
    sensorInRangeLimit=getValueFromJsonKRD(modelConfig,"sensorInRangeLimit",randomGen,10000.0);
    sensorInCoverageLimit=deg2rad(getValueFromJsonKRD(modelConfig,"sensorInCoverageLimit",randomGen,5.0));
    //(c1)射撃条件に関するパラメータ
    ...
    //(c3)離脱終了条件に関するパラメータ
    tWithdraw=getValueFromJsonKRD(modelConfig,"tWithdraw",randomGen,50.0);
    //(s1)通常時の行動選択に関するパラメータ
    pAdvanceAlly=getValueFromJsonKRD(modelConfig,"pAdvanceAlly",randomGen,60.0);
    pApproachAlly=getValueFromJsonKRD(modelConfig,"pApproachAlly",randomGen,30.0);
    pKeepSensingAlly=getValueFromJsonKRD(modelConfig,"pKeepSensingAlly",randomGen,10.0);
    pApproachMyself=getValueFromJsonKRD(modelConfig,"pApproachMyself",randomGen,30.0);
    pKeepSensingMyself=getValueFromJsonKRD(modelConfig,"pKeepSensingMyself",randomGen,70.0);
    //(s1-1)前進に関するパラメータ
    dPrioritizedAdvance=getValueFromJsonKRD(modelConfig,"dPrioritizedAdvance",randomGen,0.1);
    thetaPrioritizedAdvance=deg2rad(getValueFromJsonKRD(modelConfig,"thetaPrioritizedAdvance",randomGen,45.0));
    //(s1-3)横行に関するパラメータ
    thetaKeepSensing=deg2rad(getValueFromJsonKRD(modelConfig,"thetaKeepSensing",randomGen,10.0));
    //(s1-4)後退に関するパラメータ
    RTBEnterMargin=getValueFromJsonKRD(modelConfig,"RTBEnterMargin",randomGen,50000.0);
    RTBExitMargin=getValueFromJsonKRD(modelConfig,"RTBExitMargin",randomGen,100000.0);
    ...
    hEvasion=getValueFromJsonKRD(modelConfig,"hEvasion",randomGen,2000.0);
    //(o1)副目標の追尾に関するパラメータ
    thetaModForSensing=deg2rad(getValueFromJsonKRD(modelConfig,"thetaModForSensing",randomGen,10.0));
    //(o2)高度維持に関するパラメータ
    thetaStable=deg2rad(getValueFromJsonKRD(modelConfig,"thetaStable",randomGen,15.0));
    hNormal=getValueFromJsonKRD(modelConfig,"hNormal",randomGen,10000.0);
    nl::json sub={
        {"pGain",-3e-1},
        {"dGain",-1e-1},
        {"minPitch",-thetaStable},
        {"maxPitch",thetaStable}
    };
	altitudeKeeper=AltitudeKeeper(sub);
    //(o3)場外の防止に関するパラメータ
    dOutLimit=getValueFromJsonKRD(modelConfig,"dOutLimit",randomGen,10000.0);
    dOutLimitTurnAxis=getValueFromJsonKRD(modelConfig,"dOutLimitTurnAxis",randomGen,10000.0);
    dOutLimitKeepSensing=getValueFromJsonKRD(modelConfig,"dOutLimitKeepSensing",randomGen,20000.0);
    //目標選択に関するパラメータ(突破優先に関するもの)
    dPrioritizedAimLeaker=getValueFromJsonKRD(modelConfig,"dPrioritizedAimLeaker",randomGen,0.1);
    thetaPrioritizedAimLeaker=deg2rad(getValueFromJsonKRD(modelConfig,"thetaPrioritizedAimLeaker",randomGen,45.0));
    //最低速度維持のためのパラメータ
    ...
    nominalThrottle=getValueFromJsonKRD(modelConfig,"nominalThrottle",randomGen,0.475);
    state=State::ADVANCE;
    track.clear();
    target=TrackInfo();
...
R4InitialFighterAgent01::~R4InitialFighterAgent01(){}

void R4InitialFighterAgent01::validate(){
    ...
    if(parent->isinstance<CoordinatedFighter>()||parent->isinstance<SixDoFFighter>()){
        ...
        throw std::runtime_error("R4InitialFighterAgent01 accepts only MassPointFighrer, CoordinatedFighter, or SixDoFFighter.");
    }
}
void R4InitialFighterAgent01::perceive(bool inReset){
...
void R4InitialFighterAgent01::updateTrackInfo(TrackInfo& ti,bool isFull){
...
void R4InitialFighterAgent01::updateTargetStatus(bool isFull){
...
void R4InitialFighterAgent01::updateTargetStatusSub(TrackInfo& tgt,bool isFull){
    /*指定した目標情報を更新する。
    */
    if(!tgt.is_none()){
        bool updated=false;
...
bool R4InitialFighterAgent01::chkFuelSufficiency(){
    //余剰燃料のチェック
...
std::pair<bool,std::pair<TrackInfo,double>> R4InitialFighterAgent01::chkConditionForShoot(TrackInfo& tgt){
	/*(c1)射撃条件の判定
...
std::vector<bool> R4InitialFighterAgent01::chkBreak(){
    //(c2)離脱条件の判定
    std::vector<bool> ret;
...
bool R4InitialFighterAgent01::chkTransitionCount(){
    return waitingTransition && getT()-transitionTriggeredTime>=transitionDelay;
}
void R4InitialFighterAgent01::reserveTransition(const State& dst,double delay){
...
void R4InitialFighterAgent01::completeTransition(){
    bool isAbnormal=(
...
void R4InitialFighterAgent01::cancelTransition(){
    waitingTransition=false;
}
void R4InitialFighterAgent01::immidiateTransition(const State& dst){
    bool isAbnormal=(
...
void R4InitialFighterAgent01::selectTarget(){
...
void R4InitialFighterAgent01::decideShoot(TrackInfo& tgt){
    launchFlag=true;
    launchedTarget=tgt.copy();
    launchedTime=getT();
}
bool R4InitialFighterAgent01::chooseTransitionNormal(){
	...
	*/
    ...
    if(!chkFuelSufficiency()){
        if(transitionCause!=TransitionCause::FUEL_SHORTAGE){
...
void R4InitialFighterAgent01::deploy(py::object action){
    launchFlag=false;
    updateMyInfo();
    updateTracks(true);
...
bool R4InitialFighterAgent01::deploySub(){
	/*
...
void R4InitialFighterAgent01::control(){
	/*stateに応じた制御出力を決定する。
	...
	*/
    ...
    if(state==State::EVADE){
        ...
        altitudeKeeper.maxPitch=thetaStable;
        ...
        if(state==State::RTB){
            ...
            altitudeKeeper.minPitch=-thetaStable;
            altitudeKeeper.maxPitch=thetaStable;
        }
...
void R4InitialFighterAgent01::makeCommand(){
    double V=vel.norm();
    if(parent->isinstance<CoordinatedFighter>()||parent->isinstance<SixDoFFighter>()){
        MotionState myMotion(parent->observables.at("motion"));
        Eigen::Vector3d ex=myMotion.relBtoP(Eigen::Vector3d(1,0,0));
...
double R4InitialFighterAgent01::getT(){
    return manager->getTime();
}
double R4InitialFighterAgent01::calcRHead(const Eigen::Vector3d &rs,const Eigen::Vector3d &vs,const Eigen::Vector3d &rt,const Eigen::Vector3d &vt){
    std::shared_ptr<FighterAccessor> f=std::dynamic_pointer_cast<FighterAccessor>(parent);
    return f->getRmax(rs,vs,rt,vt,0.0);
}
double R4InitialFighterAgent01::calcRTail(const Eigen::Vector3d &rs,const Eigen::Vector3d &vs,const Eigen::Vector3d &rt,const Eigen::Vector3d &vt){
    std::shared_ptr<FighterAccessor> f=std::dynamic_pointer_cast<FighterAccessor>(parent);
    return f->getRmax(rs,vs,rt,vt,M_PI);
}

void exportR4InitialFighterAgent01(py::module& m)
{
    ...
    {
    auto cls=EXPOSE_CLASS(R4InitialFighterAgent01);
    cls
    DEF_FUNC(R4InitialFighterAgent01,validate)
    DEF_FUNC(R4InitialFighterAgent01,deploy)
    DEF_FUNC(R4InitialFighterAgent01,control)
    DEF_READWRITE(R4InitialFighterAgent01,state)
    ;
    py::enum_<R4InitialFighterAgent01::State>(cls,"State")
    .value("ADVANCE",R4InitialFighterAgent01::State::ADVANCE)
    .value("APPROACH_TARGET",R4InitialFighterAgent01::State::APPROACH_TARGET)
    .value("KEEP_SENSING",R4InitialFighterAgent01::State::KEEP_SENSING)
    .value("RTB",R4InitialFighterAgent01::State::RTB)
    .value("WITHDRAW",R4InitialFighterAgent01::State::WITHDRAW)
    .value("EVADE",R4InitialFighterAgent01::State::EVADE)
    .value("NONE",R4InitialFighterAgent01::State::NONE)
    ;
    }
}
