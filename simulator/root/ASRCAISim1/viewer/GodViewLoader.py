...
class GodViewLoader:
	...
	def __init__(self,modelConfig={},instanceConfig={}):
		...
		self.outputPrefix=modelConfig.get("outputPrefix",None) #保存先のprefixを指定する。未指定の場合は保存せず表示のみとなる。
	...
	def run(self):
		...
		for path in self.dataPaths:
			...
			with bz2.BZ2File(path,"rb") as fin:
				...
				while True:
					try:
						self.currentFrame=pickle.load(fin)
	...
	def saveImage(self):
		try:
			if(not self.asVideo):
				path=self.outputPrefix+"_e{:04d}_f{:06.1f}.png".format(self.episodeCounter,self.frameCounter)
	...
	def onEpisodeBegin(self):
		...
		if(self.asVideo):
			...
			path=self.outputPrefix+"_{}.mp4".format(self.currentFileName)
			os.makedirs(os.path.dirname(path),exist_ok=True)
	...
	def display(self,frame):
		...
		drawText2D(txt,pygame.font.Font('freesansbold.ttf',16),pos,(0,180,50,255))
		west=[agent["repr"] for agent in frame["agents"].values() if agent["team"]==ruler["westSider"]]
		east=[agent["repr"] for agent in frame["agents"].values() if agent["team"]==ruler["eastSider"]]
		...
		for fullName,f in frame["fighters"].items():
			if(f["team"]==ruler["eastSider"] and f["isAlive"]):
				...
				drawText2D(f["name"]+":v="+str(V)+",m="+str(f["remMsls"])+agTxt,self.font,(pf[0][0]+10,pf[0][1]+0),(0,0,0,255))
				drawText2D(f["name"]+":v="+str(V)+",m="+str(f["remMsls"])+agTxt,self.font,(pf[1][0]+10,pf[1][1]+0),(0,0,0,255))
				#センサ
