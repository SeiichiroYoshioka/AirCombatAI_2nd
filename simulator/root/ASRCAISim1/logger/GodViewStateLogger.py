...
class GodViewStateLogger(Callback):
	...
	def onEpisodeBegin(self):
		...
		self.initialFrame=self.makeFrame()
	...
	def makeFrame(self):
		...
		"""
		ret={"time":self.manager.getTime(),"tick":self.manager.getTickCount()}
		...
		} for agent in [a() for a in self.manager.getAgents()]
		}
		ret["totalRewards"]={k:v for k,v in self.manager.totalRewards.items()}
		...
		} for f in [f() for f in self.manager.getAssets(lambda a:isinstance(a,Fighter))]}
		...
		} for m in [m() for m in self.manager.getAssets(lambda a:isinstance(a,Missile))]}
