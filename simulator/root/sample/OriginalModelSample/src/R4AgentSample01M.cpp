...
R4AgentSample01M::ActionInfo::ActionInfo(){
	dstDir<<1,0,0;
	dstAlt=10000.0;
	velRecovery=false;
	asThrottle=false;
...
R4AgentSample01M::TeamOrigin::TeamOrigin(){
	isEastSider=true;
	pos<<0,0,0;
}
R4AgentSample01M::TeamOrigin::TeamOrigin(bool isEastSider_,double dLine){
...
R4AgentSample01M::TeamOrigin::~TeamOrigin(){}
Eigen::Vector3d R4AgentSample01M::TeamOrigin::relBtoP(const Eigen::Vector3d& v) const{//陣営座標系⇛慣性座標系
	if(isEastSider){
...
Eigen::Vector3d R4AgentSample01M::TeamOrigin::relPtoB(const Eigen::Vector3d& v) const{//慣性座標系⇛陣営座標系
	if(isEastSider){
		return Eigen::Vector3d(-v(1),v(0),v(2));
	}else{
		return Eigen::Vector3d(v(1),-v(0),v(2));
...
R4AgentSample01M::R4AgentSample01M(const nl::json& modelConfig_,const nl::json& instanceConfig_)
:Agent(modelConfig_,instanceConfig_){
	...
	assert(maxTrackNum["Friend"]>=parents.size());
	...
	mslVelNormalizer=getValueFromJsonKRD(modelConfig,"mslVelNormalizer",randomGen,1200.0);
	...
	if(use_image_observation){
		...
		image_front_range=getValueFromJsonKRD(modelConfig,"image_front_range",randomGen,200000.0);
		image_back_range=getValueFromJsonKRD(modelConfig,"image_back_range",randomGen,0.0);
		image_side_range=getValueFromJsonKRD(modelConfig,"image_side_range",randomGen,100000.0);
		image_horizon=getValueFromJsonKRD(modelConfig,"image_horizon",randomGen,120);
	...
	//  左右旋回に関する設定
	dstAz_relative=getValueFromJsonKRD(modelConfig,"dstAz_relative",randomGen,true);
	...
	accelTable=getValueFromJsonKRD<Eigen::VectorXd>(modelConfig,"accelTable",randomGen,
		(Eigen::Matrix<double,3,1>()<<-20.0,0.0,20.0).finished());
	std::sort(accelTable.begin(),accelTable.end());
	always_maxAB=getValueFromJsonKRD(modelConfig,"always_maxAB",randomGen,false);
	//  射撃に関する設定
	use_Rmax_fire=getValueFromJsonKRD(modelConfig,"use_Rmax_fire",randomGen,true);
	if(use_Rmax_fire){
	...
	dOutLimitScale=getValueFromJsonKRD(modelConfig,"dOutLimitScale",randomGen,50.0);
	//  同時射撃数の制限に関する設定
	...
	if(use_vector_observation){
		...
		enemy_dim=3+4+4+3*maxTrackNum["Friend"];
		...
		last_action_obs=Eigen::MatrixXf::Zero(maxTrackNum["Friend"],last_action_dim);
		vector_single_dim=(include_last_action ? maxTrackNum["Friend"]*last_action_dim : 0)+
    ...
    for(auto&& e:parents){
		actionInfos.push_back(ActionInfo());
	}
}
R4AgentSample01M::~R4AgentSample01M(){}

void R4AgentSample01M::validate(){
	//Rulerに関する情報の取得
    ...
    for(auto&& e:parents){
		...
		if(parent->isinstance<CoordinatedFighter>()||parent->isinstance<SixDoFFighter>()){
			...
			throw std::runtime_error("R4AgentSample01M accepts only SixDoFFighter or CoordinatedFighter for its parent.");
		}
		auto& actionInfo=actionInfos[pIdx];
		actionInfo.dstDir<<0,(getTeam()==eastSider ? -1 : +1),0;
		last_action_obs(pIdx,0)=atan2(actionInfo.dstDir(1),actionInfo.dstDir(0));
		actionInfo.lastShotTimes.clear();
		pIdx++;
	}
}
py::object R4AgentSample01M::observation_space(){
	return _observation_space;
}
py::object R4AgentSample01M::makeObs(){
	//observablesの収集
	extractObservables();
	...
	if(use_vector_observation){
		if(vector_past_points.size()>0){
			...
			//過去のobsの読み込み(リングバッファ)
			int stepCount=round(manager->getTickCount()/manager->getAgentInterval());
...
void R4AgentSample01M::extractObservables(){
	//味方機
	ourMotion.clear();
    ...
    for(auto&& e:parents){
        auto parent=e.second;
		if(parent->isAlive()){
			ourMotion.push_back(MotionState(parent->observables.at("motion")));
    ...
    for(auto&& e:parents){
		...
		if(parent->isAlive()){
    		...
    		[&](Track3D& lhs,Track3D& rhs)->bool{
    			...
    			for(auto&& e:parents){
        			auto parent_=e.second;
					if(parent_->isAlive()){
						auto myMotion=MotionState(parent_->observables.at("motion"));
...
double R4AgentSample01M::calcDistanceMargin(const MotionState& myMotion, const nl::json& myObservables){
    //正規化した余剰燃料(距離換算)の計算
...
void R4AgentSample01M::makeImageObservation(){
...
Eigen::VectorXf R4AgentSample01M::makeVectorObservation(){
	Eigen::Vector3d pos,vel;
	double V,R;
    ...
    for(auto&& e:parents){
		...
		if(parent->isAlive()){
			...
			double deltaAz=last_action_obs(pIdx,0)-myMotion.az;
			last_action_obs(pIdx,0)=atan2(sin(deltaAz),cos(deltaAz));
		}
		pIdx++;
	}
	...
	for(int tIdx=0;tIdx<lastTrackInfo.size();++tIdx){
    	...
    	for(auto&& e:parents){
			...
			if(parent->isAlive()){
				MotionState myMotion(parent->observables.at("motion"));
				enemy_temporal(tIdx,pIdx*3+11)=calcRHead(parent,myMotion,t)/horizontalNormalizer;//RHead
				enemy_temporal(tIdx,pIdx*3+12)=calcRTail(parent,myMotion,t)/horizontalNormalizer;//RTail
				enemy_temporal(tIdx,pIdx*3+13)=calcRNorm(parent,myMotion,t);//現在の距離をRTail〜RHeadで正規化したもの
			}
		}
	}
	...
	//observationの生成
	Eigen::VectorXf ret(
		(include_last_action ? maxTrackNum["Friend"]*last_action_dim : 0)+
		maxTrackNum["Friend"]*friend_dim+
		maxTrackNum["Enemy"]*enemy_dim+
		maxMissileNum["Friend"]*friend_msl_dim
	);
	...
	if(include_last_action){
		d=maxTrackNum["Friend"]*last_action_dim;
...
py::object R4AgentSample01M::action_space(){
	return _action_space;
}

void R4AgentSample01M::deploy(py::object action_){
	last_action_obs=Eigen::MatrixXf::Zero(maxTrackNum["Friend"],last_action_dim);
    ...
    for(auto&& e:parents){
		...
		last_action_obs(pIdx,1)=use_altitude_command ? actionInfo.dstAlt : dstPitch;
		...
		if(always_maxAB){
			...
			last_action_obs(pIdx,2)=accel/std::max(accelTable(accelTable.size()-1),accelTable(0));
		}
		//下限速度の制限
		if(V<minimumV){
		...
		if(
			parent->observables.at("/weapon/remMsls"_json_pointer)>0 && 
			parent->observables.at("/weapon/launchable"_json_pointer) && 
			shotTarget>=0 && 
			...
			if(use_Rmax_fire){
				...
				double r=calcRNorm(parent,myMotion,t);
				if(r<=shotThreshold){
					//射程の条件を満たしている
		...
		last_action_obs(pIdx,3+(shotTarget+1))=1;
		if(shotTarget>=0){
...
void R4AgentSample01M::control(){
    ...
    for(auto&& e:parents){
		...
		//目標方位の補正
		if(abs(pos(0))>=(1-2*(1-dOutLimitRatio))*dOut){
			//戦域逸脱を避けるための方位補正
			//判定ラインの超過具合に応じて制限角度を変化させる。(無限遠でラインに直交、ライン上でラインと平行)
			double over=dOutLimitScale*(abs(pos(0))/dOut-dOutLimitRatio);
			...
			if(pos(0)>0){//北側
				if(vel(1)>0){//現在東向き
					//南東側のみ許容
					if(actionInfo.dstDir(1)<0){
						//西を向きたい場合、dstDirは真南を限度とする
						actionInfo.dstDir=Eigen::Vector3d(-n,0,actionInfo.dstDir(2));
					}else{
						//東を向きたい場合、角度を制限する
						if(atan2(-actionInfo.dstDir(0),actionInfo.dstDir(1))<theta){
							actionInfo.dstDir=Eigen::Vector3d(-n*sn,n*cs,actionInfo.dstDir(2));
						}
					}
				}else{//現在西向き
					//南西側のみ許容
					if(actionInfo.dstDir(1)>0){
						//東を向きたい場合、dstDirは真南を限度とする
						actionInfo.dstDir=Eigen::Vector3d(-n,0,actionInfo.dstDir(2));
					}else{
						//西を向きたい場合、角度を制限する
						if(atan2(-actionInfo.dstDir(0),-actionInfo.dstDir(1))<theta){
...
py::object R4AgentSample01M::convertActionFromAnother(const nl::json& decision,const nl::json& command){
    ...
    for(auto&& e:parents){
		...
		if(use_Rmax_fire){
			if(shotTarget_ex<0){
				...
				if(shotThresholdTable.size()>1){
					double r=calcRNorm(parent,myMotion,expertTarget);
					unflattened_action(action_idx)=shotThresholdTable.size()-1;
...
void R4AgentSample01M::controlWithAnotherAgent(const nl::json& decision,const nl::json& command){
...
int R4AgentSample01M::flatten_action(const Eigen::VectorXi& original_action,const Eigen::VectorXi& original_shape){
...
Eigen::VectorXi R4AgentSample01M::unflatten_action(const int& flattened_action,const Eigen::VectorXi& original_shape){
...
bool R4AgentSample01M::isInside(const int& lon, const int& lat){
    //ピクセル座標(lon,lat)が画像の範囲内かどうかを返す。
	return 0<=lon && lon<image_longitudinal_resolution && 0<=lat && lat<image_lateral_resolution;
}
Eigen::Vector2i R4AgentSample01M::rposToGrid(const Eigen::Vector3d& dr){
...
Eigen::Vector3d R4AgentSample01M::gridToRpos(const int& lon, const int& lat, const double& alt){
...
Eigen::Vector2i R4AgentSample01M::posToGrid(const Eigen::Vector3d& pos){
	//慣性座標系での絶対位置posに対応するピクセル座標(lon,lat)を返す。
	//自陣営防衛ライン中央が基準点
	Eigen::Vector3d rpos=pos-teamOrigin.pos;
	//慣性座標系から陣営座標系に回転
	rpos=teamOrigin.relPtoB(rpos);
	return rposToGrid(rpos);
}
Eigen::Vector3d R4AgentSample01M::gridToPos(const int& lon, const int& lat, const double& alt){
...
void R4AgentSample01M::plotPoint(const Eigen::Vector3d& pos,const int& ch, const float& value){
...
void R4AgentSample01M::plotLine(const Eigen::Vector3d& pBegin,const Eigen::Vector3d& pEnd,const int& ch, const float& value){
...
double R4AgentSample01M::calcRHead(std::shared_ptr<PhysicalAssetAccessor> parent,const MotionState& myMotion,const Track3D& track){
...
double R4AgentSample01M::calcRTail(std::shared_ptr<PhysicalAssetAccessor> parent,const MotionState& myMotion,const Track3D& track){
	std::shared_ptr<FighterAccessor> f=std::dynamic_pointer_cast<FighterAccessor>(parent);
	Eigen::Vector3d rt=track.posI();
	Eigen::Vector3d vt=track.velI();
	Eigen::Vector3d rs=myMotion.pos;
...
double R4AgentSample01M::calcRNorm(std::shared_ptr<PhysicalAssetAccessor> parent,const MotionState& myMotion,const Track3D& track){
	double RHead=calcRHead(parent,myMotion,track);
	double RTail=calcRTail(parent,myMotion,track);
	Eigen::Vector3d rs=myMotion.pos;
	Eigen::Vector3d rt=track.posI();
...
void exportR4AgentSample01M(py::module &m)
{
	using namespace pybind11::literals;
	EXPOSE_LOCAL_CLASS(R4AgentSample01M)
	DEF_FUNC(R4AgentSample01M,validate)
	DEF_FUNC(R4AgentSample01M,observation_space)
	DEF_FUNC(R4AgentSample01M,makeObs)
	DEF_FUNC(R4AgentSample01M,flatten_action)
	DEF_FUNC(R4AgentSample01M,unflatten_action)
	DEF_FUNC(R4AgentSample01M,deploy)
	DEF_FUNC(R4AgentSample01M,control)
	DEF_FUNC(R4AgentSample01M,convertActionFromAnother)
	.def("convertActionFromAnother",[](R4AgentSample01M& v,const py::object& decision,const py::object& command){
