...
namespace pybind11{
    namespace detail{
        ...
        template <typename Scalar_, int NumIndices_, int Options_> struct type_caster<Eigen::Tensor<Scalar_,NumIndices_,Options_>>
        {
        public:
            typedef typename Eigen::Tensor<Scalar_,NumIndices_,Options_> TensorType;
            PYBIND11_TYPE_CASTER(TensorType, _("Eigen::Tensor"));

            static handle eigen_tensor_cast(TensorType const &src, handle base = handle(), bool writeable = true) {
                constexpr ssize_t elem_size = sizeof(Scalar_);
                auto d=src.dimensions();
                std::vector<Eigen::Index> dimensions(d.size()),strides(d.size());
                for(int i=0;i<d.size();++i){
                    dimensions[i]=d[i];
                }
                if(Options_ & Eigen::RowMajor){
                    int s=elem_size;
                ...
                py::array a;
                a = array(dimensions, strides, src.data(), base);
                return a.release();
            }
            static handle eigen_ref_tensor(TensorType &src, handle parent = none()) {
                return eigen_tensor_cast(src, parent, !std::is_const<TensorType>::value);
            }
            bool load(handle src, bool)
            {
                ...
                typename TensorType::Dimensions dimensions;
                for(int i=0;i<dims;++i){
                    dimensions[i]=buf.shape(i);
                }
                value=TensorType(dimensions);
                auto ref = reinterpret_steal<array>(eigen_ref_tensor(value));
